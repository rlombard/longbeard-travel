...
public class OpenAiLlmProviderService(HttpClient httpClient, IOptions<OpenAiSettings> settings, IOptions<LlmSettings> llmSettings) : ILlmProviderService
{
    ...
    public async Task<LlmProviderResponse> GenerateAsync(LlmRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var endpoint = string.IsNullOrWhiteSpace(settings.Value.BaseUrl) ? "https://api.openai.com/v1/chat/completions" : $"{settings.Value.BaseUrl!.TrimEnd('/')}/chat/completions";
